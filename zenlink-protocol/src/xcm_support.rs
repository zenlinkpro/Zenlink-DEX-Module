...
use codec::{Decode};
use crate::{
    AssetId, Junction, LocationConversion, MultiAsset, MultiLocation, ParaId, TokenBalance,
    TransactAsset, XcmError, XcmResult, ZenlinkMultiAsset, FilterAssetLocation,
...
> Transactor<NativeCurrency, ZenlinkAssets, AccountIdConverter, AccountId, ParaChainId>
{
    ...
    fn deposit_abstract_asset(id: &[u8], who: &AccountId, amount: u128) -> XcmResult {
        ...
        let para_id: u32 = ParaChainId::get().into();
        match operate {
            CrossChainOperation::AddLiquidity { origin_chain, target_chain, token_0, token_1, amount_1 } => {
                sp_std::if_std! {
                ...
                match para_id {
                    ...
                    // Remote execution
                    x if x == target_chain => {
                        ZenlinkAssets::deposit(AssetId::from(token_0), &who, amount as TokenBalance)
                            .map_err(|err| {
                                sp_std::if_std! { println!("zenlink::<AddLiquidity> err = {:?}", err); }
                                XcmError::Undefined
                            })?;

                        let despoit_amount = ZenlinkAssets::balance_of(AssetId::from(token_0), &who);
            ...
            CrossChainOperation::SwapExactTokensForTokens { origin_chain, target_chain, amount_out_min, path } => {
                sp_std::if_std! {
                ...
                match para_id {
                    ...
                    x if x == target_chain => {
                        ...
                        ZenlinkAssets::swap_in(&who, despoit_amount, amount_out_min, &asset_id_path)
                            .map_err(|err| {
                                sp_std::if_std! { println!("zenlink::<SwapExactTokensForTokens> err = {:?}", err); }
                                XcmError::Undefined
                            })?;
            ...
            CrossChainOperation::SwapTokensForExactTokens { origin_chain, target_chain, amount_out, path } => {
                sp_std::if_std! {
                ...
                match para_id {
                    ...
                    x if x == target_chain => {
                        ...
                        ZenlinkAssets::swap_out(&who, amount_out, amount as TokenBalance, &asset_id_path).map_err(|err| {
                            sp_std::if_std! { println!("zenlink::<SwapTokensForExactTokens> err = {:?}", err); }
                            XcmError::Undefined
                        })?;
    ...
    fn withdraw_abstract_asset(id: &[u8], who: &AccountId, amount: u128) -> Result<MultiAsset, XcmError> {
        ...
        let _ = NativeCurrency::withdraw(&who, value, WithdrawReasons::TRANSFER, ExistenceRequirement::AllowDeath)
            .map_err(|_| XcmError::Undefined)?;

        sp_std::if_std! { println!("zenlink::<withdraw_abstract_asset> success"); }

        Ok(MultiAsset::AbstractFungible { id: id.to_vec(), amount })
    }
}
