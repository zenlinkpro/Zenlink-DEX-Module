...
impl TryFrom<CurrencyId> for AssetId {
    ...
    fn try_from(id: CurrencyId) -> Result<Self, Self::Error> {
        let index = match id {
            CurrencyId::Native(token_symbol) => {
                Ok((0 as u64, token_symbol as u64))
            }
            CurrencyId::VToken(token_symbol) => {
                Ok((1 as u64, token_symbol as u64))
            }
            CurrencyId::Token(token_symbol) => {
                Ok((2 as u64, token_symbol as u64))
            }
            CurrencyId::Stable(token_symbol) => {
                Ok((3 as u64, token_symbol as u64))
            }
            CurrencyId::VSToken(token_symbol) => {
                Ok((4 as u64, token_symbol as u64))
            }
            CurrencyId::LPToken(symbol0, index0, symbol1, index1) => {
                let currency_index0 = (((((index0 as u64) << 8) & 0x0000_ff00) + (symbol0 as u64 & 0x0000_00ff)) as u64) << 16;
                let currency_index1 = (((((index1 as u64) << 8) & 0x0000_ff00) + (symbol1 as u64 & 0x0000_00ff)) as u64) << 32;
        ...
        return if id.is_native() {
            Ok(AssetId { chain_id: BIFROST_PARACHAIN_ID, asset_type: NATIVE, asset_index })
        } else {
            Ok(AssetId {
