...
const BASE_VIRTUAL_PRICE_PRECISION: Balance = 1_000_000_000_000_000_000;
...
const MINUTE: u64 = 3600;
const BASE_CACHE_EXPIRE_TIME: u64 = 10 * MINUTE;
...
impl<T: Config> Pallet<T> {
	...
	fn meta_pool_add_liquidity(
		...
		let base_virtual_price = Self::update_meta_pool_virtual_price(meta_pool).ok_or(Error::<T>::Arithmetic)?;
		...
		d1 = Self::get_d(&normalized_balances, amp).ok_or(Error::<T>::Arithmetic)?;
		ensure!(d1 > d0, Error::<T>::CheckDFailed);
	...
	fn update_meta_pool_virtual_price(
		meta_pool: &mut MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
	) -> Option<Balance> {
	...
	fn get_pool_virtual_price(
		pool: &Pool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
	) -> Option<Balance> {
		match pool {
			Pool::Basic(bp) => Self::calculate_base_pool_virtual_price(bp),
			Pool::Meta(mp) => Self::get_meta_pool_virtual_price(mp),
		}
	}

	fn get_meta_pool_virtual_price(
		meta_pool: &MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
	) -> Option<Balance> {
		let base_virtual_price = Self::get_meta_pool_base_virtual_price(meta_pool)?;
		let normalized_balances = Self::meta_pool_xp(
			&meta_pool.info.balances,
			&meta_pool.info.token_multipliers,
			base_virtual_price,
	...
	fn get_meta_pool_base_virtual_price(
		meta_pool: &MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
	) -> Option<Balance> {
		let now = T::TimeProvider::now().as_secs();
		if now > (meta_pool.base_cache_last_updated + BASE_CACHE_EXPIRE_TIME) {
		...
		return Some(meta_pool.base_virtual_price);
	}
	...
	fn meta_pool_swap(
		...
		let virtual_price = Self::update_meta_pool_virtual_price(meta_pool).ok_or(Error::<T>::Arithmetic)?;
	...
	fn base_pool_remove_liquidity_one_currency(
		pool_id: T::PoolId,
		pool: &mut BasePool<T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		who: &T::AccountId,
		lp_amount: Balance,
	...
	fn meta_pool_remove_liquidity_one_currency(
		pool_id: T::PoolId,
		meta_pool: &mut MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		who: &T::AccountId,
		lp_amount: Balance,
		...
		Self::update_meta_pool_virtual_price(meta_pool).ok_or(Error::<T>::Arithmetic)?;

		let (dy, dy_fee) =
			Self::calculate_meta_pool_remove_liquidity_one_currency(meta_pool, lp_amount, index as usize, total_supply)
				.ok_or(Error::<T>::Arithmetic)?;
	...
	pub(crate) fn calculate_meta_pool_remove_liquidity_one_currency(
		meta_pool: &MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		...
		for (i, xpi) in xp.iter().enumerate() {
			let dx_expected: U256;
			let u256_xpi = U256::from(*xpi);
			if i == index {
				dx_expected = u256_xpi
	...
	fn base_pool_remove_liquidity_imbalance(
		who: &T::AccountId,
		pool_id: T::PoolId,
		pool: &mut BasePool<T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		amounts: &[Balance],
		...
		let (mut burn_amount, fees, d1) =
			Self::calculate_remove_liquidity_imbalance(pool, amounts, total_supply).ok_or(Error::<T>::Arithmetic)?;
	...
	fn meta_pool_remove_liquidity_imbalance(
		who: &T::AccountId,
		pool_id: T::PoolId,
		meta_pool: &mut MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		amounts: &[Balance],
		...
		let (mut burn_amount, fees, d1) =
			Self::calculate_meta_pool_remove_liquidity_imbalance(meta_pool, &amounts, total_supply, base_virtual_price)
				.ok_or(Error::<T>::Arithmetic)?;
	...
	fn inner_remove_pool_and_base_pool_liquidity_one_currency(
		...
		let base_pool = Self::pools(base_pool_id).ok_or(Error::<T>::InvalidPoolId)?;
		let base_pool_lp_currency = base_pool.clone().get_lp_currency();
		let base_pool_currencies = base_pool.get_currency_ids();

		let mut base_pool_currency_index: Option<u32> = None;
		for (i, c) in base_pool_currencies.iter().enumerate() {
			if *c == base_pool_lp_currency {
	...
	fn inner_swap_pool_from_base(
		...
		Pools::<T>::try_mutate_exists(pool_id, |optioned_pool| -> Result<Balance, DispatchError> {
			...
			match pool {
				Pool::Meta(mp) => {
					...
					ensure!(base_pool_id == mp.base_pool_id, Error::<T>::MismatchParameter);

					let currency_index_from = (in_index as usize)
						.checked_add(meta_pool_lp_index)
						.ok_or(Error::<T>::Arithmetic)?;
					let currency_index_to = out_index as usize;

					Self::meta_pool_swap_underlying(
						mp,
						pool_id,
						who,
						to,
		...
		// 	Self::get_currency_index(pool_id,
		// base_pool_currency).ok_or(Error::<T>::InvalidBasePool)?;
		//
	...
	fn inner_swap_pool_to_base(
		...
		// }
		// let out_amount =
		// 	Self::inner_remove_liquidity_one_currency(base_pool_id, who, base_lp_amount, out_index,
		// min_dy, to)?;
		//
		// Ok(out_amount)
	}
	...
	fn calculate_meta_pool_remove_liquidity_imbalance(
		meta_pool: &mut MetaPool<T::PoolId, T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
		amounts: &[Balance],
		total_supply: Balance,
		base_virtual_price: Balance,
	...
	pub(crate) fn calculate_currency_amount(
		...
		if let Some(pool) = Self::pools(pool_id) {
			match pool {
				Pool::Basic(bp) => {
					Self::calculate_base_currency_amount(&bp, amounts, deposit)
				}
				Pool::Meta(mp) => {
					Self::calculate_meta_currency_amount(&mp, amounts, deposit)
				}
			}
		} else {
	...
	pub(crate) fn calculate_meta_currency_amount(
		...
		let base_virtual_price = Self::get_meta_pool_base_virtual_price(meta_pool).ok_or(Error::<T>::Arithmetic)?;
		let mut new_balances = meta_pool.info.balances.clone();
		let token_multipliers = meta_pool.info.token_multipliers.clone();

		let xp = Self::meta_pool_xp(&new_balances, &token_multipliers, base_virtual_price).ok_or(Error::<T>::Arithmetic)?;
		...
		let xp1 =  Self::meta_pool_xp(&new_balances, &token_multipliers, base_virtual_price).ok_or(Error::<T>::Arithmetic)?;
	...
	#[allow(dead_code)]
	pub(crate) fn calculate_base_pool_virtual_price_with_id(pool_id: T::PoolId) -> Option<Balance> {
		if let Some(pool) = Self::pools(pool_id) {
			match pool {
				Pool::Basic(bp) => {
					Self::calculate_base_pool_virtual_price(&bp)
				}
				_ =>None
			}
		} else {
	...
	pub(crate) fn calculate_base_pool_virtual_price(
		pool: &BasePool<T::CurrencyId, T::AccountId, BoundedVec<u8, T::PoolCurrencySymbolLimit>>,
	) -> Option<Balance> {
		let d = Self::get_d(
